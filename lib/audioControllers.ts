...
// export async function getMp3FromYouTube(
//   youtubeUrl: string,
//   downloadId?: string
// ): Promise<{
//   base64Buffer: string;
...
export async function saveMp3ToDevice({
  ...
  try {
    ...
    // Create the raw file path (without file://)
    const rawFilePath = `${DocumentDir}/downloads/${title}.mp3`;

    // Save the file using the raw path
    await RNFetchBlob.fs.writeFile(rawFilePath, base64Buffer, "base64");

    // ✅ FIX: Add file:// prefix for TrackPlayer compatibility
    const fileUrl = `file://${rawFilePath}`;

    const offlineSong: offlineSong = {
      id,
      title,
      artist: author || "Unknown",
      url: fileUrl, // ✅ Now includes file:// prefix
    };
    console.log("Saving song with URL:", offlineSong);

    pushNewSong(offlineSong);

    console.log("✅ Saved song with URL:", fileUrl);
    return offlineSong;
  } catch (error) {
    console.log("Error saving file:", error);
  }
...
export async function listDownloadedSongs() {
  ...
  try {
    // 1. Get raw list of all items in downloads/
    const allItems = await fs.ls(downloadDir); // returns string[]
