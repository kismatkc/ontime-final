  ...
  const DecisionDialog = () => (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className=" sm:max-w-[450px]">
        <DialogHeader>
          <DialogTitle>
            <Text className="text-2xl font-bold ">Log today</Text>
          </DialogTitle>
        </DialogHeader>
        <DialogFooter className="flex flex-row gap-x-6 gap-y-3">
          <TouchableOpacity
            className="bg-purple-600 rounded-md px-5 py-4 flex flex-row items-center gap-x-1 "
            onPress={handleFailure}
          >
            <Text className="text-lg font-semibold">FAILURE</Text>
            <ThumbsDownIcon color={"purple"} />
          </TouchableOpacity>

          <TouchableOpacity
            className="bg-green-600 rounded-md px-5 py-2 flex flex-row items-center gap-x-1"
