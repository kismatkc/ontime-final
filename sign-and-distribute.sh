...
# # Unlock the keychain and set timeout
# security unlock-keychain -p "$KEYCHAIN_PASSWORD" "$TEMP_KEYCHAIN"
# security set-keychain-settings -t 3600 -l "$TEMP_KEYCHAIN"

# # Import the certificate into the temporary keychain
# log "Importing certificate into temporary keychain..."
# security import "$P12_FILE" -k "$TEMP_KEYCHAIN" -P "$P12_PASSWORD" -T /usr/bin/codesign || {
#   log_error "Failed to import certificate!"
#   exit 1
# }

# # Set partition list to avoid UI prompts
# security set-key-partition-list -S apple-tool:,apple: -s -k "$KEYCHAIN_PASSWORD" "$TEMP_KEYCHAIN" > /dev/null 2>&1 || {
#   log_warning "Failed to set partition list but continuing anyway"
# }
